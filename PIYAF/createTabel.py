...
def createNewTable(spisok, workSheet, path):
    ...
    for i in range(len(spisok)):
        ...
        workerName = f'{worker[1]} {worker[2][0]}.' + ('' if worker[3] is None else (worker[3][0] + '.'))
...
def createMainHead(workSheet, month):
    n = excel_format.getMonth(month=month, arg='count')
    column = get_column_letter(n + 6)
    alignment = st.Alignment(horizontal='center', vertical='center')
    font = st.Font(
        size=14,
        bold=True,
